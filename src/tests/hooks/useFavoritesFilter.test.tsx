...
const wrapper = ({children}:{children:React.ReactNode}) => (
    <FavoritesContext.Provider value= {{ favorites: favorites, setFavorites: () => { } }}>
        {children}
...
describe("useFavoritesFilter", () => {
    test("Filter initial state should be false", () => {
        ...
        const { result } = renderHook(() => useFavoritesFilter({ data: testDataCharactaersList.data, favoritesFilter: "" }), { wrapper });

        expect(result.current.isFavoritesFiltered).not.toBeTruthy();
    });

    test("Filter active but the list of favorites emty", () => {
        const { result} = renderHook(() => useFavoritesFilter({ data: testDataCharactaersList.data, favoritesFilter: "favorites" }), { wrapper });
