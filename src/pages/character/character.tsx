...
export const Character: React.FC = () => {
    ...
    const { data, isloading } = useApi<CharacterResult[]>(
        path,
        optionAxios.current
    )
                            ...
                            <div className="character-border">
                                <div className="character__infromation_container">
                                    <div
                                        ...
                                        data-testid="character__img"
                                    >
                                        <img
                                            loading="lazy"
                                            src={`${data?.results[0].thumbnail.path}/portrait_fantastic.${data?.results[0].thumbnail.extension}`}
                                            alt={`${data?.results[0].name} image}`}
                                        />
                                    </div>
                                    <div className="character__information">
                                        <div className="character__name-container">
                                            <h2 className="character__name text--primary-color roboto-condensed--700">
                                                {data?.results[0].name}
                                            </h2>
                                            <FavButton id={parseInt(id)} />
                                        </div>
                                        <p className="character__description roboto-condensed--700">
